...
local serverSwitchInProgress = false
...
function LoginWindowHandler.SwitchServer(address, port)
	local Configuration = WG.Chobby.Configuration

	if serverSwitchInProgress then
		Spring.Echo("SwitchServer: switch already in progress")
		return
	end

	if not address or address == "" then
		Spring.Echo("SwitchServer: invalid address")
		return
	end

	port = port or Configuration:GetServerPort()

	if Configuration:GetServerAddress() == address
			and Configuration:GetServerPort() == port
			and lobby:GetConnectionStatus() == "connected" then
		Spring.Echo("SwitchServer: already connected to", address, port)
		return
	end
	...
	Configuration:SetRuntimeServer(address, port)

	local function reconnect()
		Spring.Echo("SwitchServer: reconnecting to", address, port)

		local shouldAutoLogin =
			Configuration.autoLogin
			or Configuration.rememberPassword

		if shouldAutoLogin and Configuration.userName then
			if not TrySimpleSteamLogin() then
				TrySimpleLogin()
			end
		else
	...
	if status == "connected" or status == "connecting" then
		local onSwitchDisconnected

		onSwitchDisconnected = function()
			lobby:RemoveListener("OnDisconnected", onSwitchDisconnected)

			WG.Delay(reconnect, 3)
		end

		lobby:AddListener("OnDisconnected", onSwitchDisconnected)

		if WG.Chobby
				and WG.Chobby.interfaceRoot
				and WG.Chobby.interfaceRoot.CleanMultiplayerState then
			WG.Chobby.interfaceRoot.CleanMultiplayerState()
		end

		lobby:Disconnect("Switching TEI server")
	else
		reconnect()
	end
end
